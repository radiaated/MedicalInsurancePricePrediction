...
df.drop(columns=["charges", "package_Bronze", "package_Platinum", "package"], inplace=True)

train, test = train_test_split(df, test_size=0.2, random_state=42)

gb_obj = GB(train, "package_Gold", 0.1, EST)
...
test["package_Gold"] = test["package_Gold"].map({True: "yes", False: "no"})

predicted_result["Predicted"] = predicted_result["Predicted"].map({True: "yes", False: "no"})



print("Accurancy: ", accuracy_score(test["package_Gold"], predicted_result["Predicted"]))
